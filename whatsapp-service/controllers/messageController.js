...
export const sendSingle = async (req, res) => {
  ...
  if (!to || !message)
    return res.status(400).json({ status: "error", message: "'to' and 'message' are required" });

  const result = await sendSingleMessage({ to, text: message });

  if (result.success)
    return res.json({ status: "success", message: "Message sent ✅", messageId: result.messageId });
  else
    return res.status(500).json({ status: "error", message: "Failed to send message", error: result.error });
};
...
export const sendBulk = async (req, res) => {
  ...
  /*
    messages format example from frontend:
    [
      { to: "+250795171169", message: "Hello Eric!" },
      { to: "+250788222222", message: "Hello Aline!" }
    ]
  */
